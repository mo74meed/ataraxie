...
setTimeout(() => {
    ...
    if (loaderFn) {
        ...
        (function () {
            ...
            function syncCloud() { if (isCloud()) window.FirebaseAuthManager.forceSync(); }
            ...
            function sv() { 
                ...
                sT = setTimeout(() => { 
                    try { 
                        if (isCloud()) { window.FirebaseAuthManager.saveProgress(profileSK(activeProfileId), st); }
            ...
            function showInlinePrompt(title, defaultVal="") {
                return new Promise(resolve => {
                    const ov = document.getElementById('customPromptOverlay');
                    ...
                    const cleanup = () => { ov.style.display = 'none'; btnC.removeEventListener('click', onCancel); btnO.removeEventListener('click', onOk); };
                    const onCancel = () => { cleanup(); resolve(null); };
                    const onOk = () => { cleanup(); resolve(inp.value.trim()); };
                    
                    const btnC = document.getElementById('customPromptCancel');
                    const btnO = document.getElementById('customPromptConfirm');
                    btnC.addEventListener('click', onCancel);
                    btnO.addEventListener('click', onOk);
                });
            }
            function showInlineConfirm(title, message) {
                return new Promise(resolve => {
                    const ov = document.getElementById('customConfirmOverlay');
                    ...
                    const btnC = document.getElementById('customConfirmCancel');
                    const btnO = document.getElementById('customConfirmConfirm');
                    btnC.addEventListener('click', onCancel);
                    btnO.addEventListener('click', onOk);
                });
            }
            ...
            function duplicateProfile(id) {
                ...
                if (!src) return;
                const data = (id === activeProfileId) ? st : JSON.parse(localStorage.getItem(profileSK(id)) || JSON.stringify(emptyState()));
            ...
            /* ══════════ HOME / HISTORY ══════════ */
                ...
                const actBlock = document.querySelector('.sb-ch.active'); if(actBlock) actBlock.classList.remove('active');
            ...
            function showResetTab(tab) {
                ...
                if (tab === 'all') {
                    ...
                    document.getElementById('reset-all-btn').onclick = () => {
                        if (confirm('Êtes-vous sûr de vouloir tout effacer ?')) { st = emptyState(); svNow()?.then(() => { closeResetModal(); window.refreshUI(); }); }
                    };
                } else {
                    foot.style.display = '';
                    ...
                    document.getElementById('reset-apply').onclick = () => {
                        ...
                        svNow()?.then(() => { closeResetModal(); window.refreshUI(); });
                    };
                }
            }
            ...
            function renderProfileList() {
                ...
                body.querySelectorAll('[data-action="rename"]').forEach(btn => {
                    btn.addEventListener('click', async (e) => { e.stopPropagation(); const n = prompt('Nouveau nom:'); if (n && n.trim()) { renameProfile(btn.dataset.id, n.trim()); renderProfileList(); } });
                });
            ...
            function init() {
                ...
                if (!D || D.length === 0) {
                    ...
                    <div class="w-icon" style="background:rgba(239,68,68,.08)"><svg style="color:#ef4444" fill="none" stroke="currentColor" viewBox="0 0 24 24" width="34" height="34"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg></div>
                    <h2>Base de données introuvable</h2>
                    <p>Le fichier de questions n'a pas pu être chargé. Vérifiez votre connexion et rechargez la page.</p>
                    `;
                ...
                if (window.FirebaseAuthManager) {
                    ...
                    window.FirebaseAuthManager.init((cloudState, user) => {
                        ...
                        if (user) {
                            ...
                            if (ak) { 
                                function wire() {
                                    // Event listener on mc removed: moved to init() for one-time registration
                                }
                                const ps = ak.split('§'); 
                                render(ps[0], ps[1]); 
                            } else {
                ...
                // Moved from wire() to init() to prevent listener stacking
                document.getElementById('mc').addEventListener('click', (e) => {
                    const h = e.target.closest('.his-pop'); if (h) go(h.dataset.m, h.dataset.c, h.dataset.q);
                });
            ...
            document.getElementById('si').addEventListener('input', (e) => {
                ...
                // Global his-pop dismiss (registered once here, not in wire())
                ...
                const sbDot = document.querySelector('.sb-foot .dot');
                if (sbDot) {
                    const updateOfflineUI = () => {
                        if (navigator.onLine) {
                            sbDot.style.background = 'var(--g500)';
                            sbDot.style.boxShadow = '0 0 8px var(--g500)';
                        } else {
                            sbDot.style.background = '#f59e0b';
                            sbDot.style.boxShadow = '0 0 8px #f59e0b';
                        }
                    };
            ...
            function buildSB() {
                ...
                mods.forEach((ents, mn) => {
                    ...
                    btn.addEventListener('click', () => { const o = cl.classList.toggle('open'); btn.classList.toggle('exp', o); if (o) cl.style.maxHeight = cl.scrollHeight + 'px'; else cl.style.maxHeight = null; st.sb[mn] = o; sv() });
            ...
            function filter(e) {
                ...
                document.querySelectorAll('.sb-mod').forEach(item => {
                    ...
                    if (q && any) { const cl = item.querySelector('.sb-chaps'); if (cl) { cl.classList.add('open'); requestAnimationFrame(() => cl.style.maxHeight = cl.scrollHeight + 'px'); } const b = item.querySelector('.sb-mod-btn'); if (b) b.classList.add('exp') }
                });
            }
        ...
        (function() {
            let highlightsArr = [];
            const highlightRoot = document.body;

            function getOffsetsFromRange(rootNode, range) {
                const preSelectionRange = range.cloneRange();
                preSelectionRange.selectNodeContents(rootNode);
            ...
            function createRangeFromOffsets(rootNode, start, end) {
                ...
                while ((node = walker.nextNode())) {
                    ...
                    if (startFound && !endFound && end <= currentOffset + nodeLength) {
                ...
                if (!startFound) {
                    ...
                    else range.selectNodeContents(rootNode);
                }
                if (!endFound) {
                    if (node) range.setEnd(node, node.nodeValue.length);
                    else range.selectNodeContents(rootNode);
                }
            ...
            function renderHighlights() {
                ...
                const highlightRanges = highlightsArr.map(pos => {
                    try {
                        return createRangeFromOffsets(highlightRoot, pos.start, pos.end);
                    } catch(e) {
                        return null;
                    }
                }).filter(Boolean);
                ...
                if (highlightRanges.length > 0) {
                    ...
                    CSS.highlights.set('selection-highlight', highlight);
                }
            }
            ...
            function handleSelection() {
                ...
                if (!highlightRoot.contains(range.commonAncestorContainer)) return;

                let { start, end } = getOffsetsFromRange(highlightRoot, range);
                if (start > end) {
                    const temp = start;
                    start = end;
                    end = temp;
                ...
                if (overlaps.length > 0) {
                    for (let i = overlaps.length - 1; i >= 0; i--) {
                        highlightsArr.splice(overlaps[i], 1);
                    }
                } else {
                    highlightsArr.push({ start, end });
                }
            ...
            highlightRoot.addEventListener('mouseup', handleSelection);
            highlightRoot.addEventListener('touchend', handleSelection);
            highlightRoot.addEventListener('keyup', handleSelection);
        })();
